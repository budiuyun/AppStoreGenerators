document.addEventListener('DOMContentLoaded', function () {
    ...
    const getFormJsonBtn = document.getElementById('get-form-json-btn');
    const jsonInput = document.getElementById('json-input');

    // 当切换到JSON标签页时，自动填充当前表单数据
    document.querySelector('.tab-btn[data-tab="json"]').addEventListener('click', () => {
        // 如果JSON文本框为空，则自动填充当前表单数据
    ...
    // 获取当前表单JSON
    getFormJsonBtn.addEventListener('click', () => {
        try {
            const formData = getFormData();
            jsonInput.value = JSON.stringify(formData, null, 2);
        } catch (error) {
    ...
    clearJsonBtn.addEventListener('click', () => {
        if (jsonInput.value.trim() && !confirm('确定要清空当前JSON内容吗？')) {
            return;
        }
        jsonInput.value = '';
    });
